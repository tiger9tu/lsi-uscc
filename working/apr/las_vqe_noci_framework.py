#!/usr/bin/env python3
"""
Unified LAS-VQE-NOCI Framework

This framework provides a systematic approach to run LAS-VQE-NOCI calculations
on multiple molecular systems with standardized configuration and testing.
"""
...
class LASVQENOCIFramework:
    """Unified framework for LAS-VQE-NOCI calculations"""
    
    def __init__(self, gradient_threshold: float = 0.0001, vqe_max_cycles: int = 10, verbose: int = 1):
        """
    ...
    def run_single_molecule(self, mol_config: Dict[str, Any]) -> LASVQENOCIResult:
        ...
        try:
            ...
            result.excited_energy = eigvals[1]
            result.ground_state_coeff1 = eigvecs[0, 0]
            result.ground_state_coeff2 = eigvecs[1, 0]
    ...
    def print_summary_table(self, results: List[LASVQENOCIResult]):
        ...
        # Header
        header = f"{'Molecule':<15} {'Basis':<8} {'Success':<8} {'LASSCF':<12} {'CASCI':<12} {'VQE-NOCI':<12} {'Gap(eV)':<8} {'Corr(mEh)':<10} {'Time(s)':<8}"
        print(header)
        print('-' * 120)
        ...
        for result in results:
            ...
            row = f"{result.mol_name:<15} {result.basis:<8} {success_str:<8} {lasscf_str:<12} {casci_str:<12} {vqe_noci_str:<12} {gap_str:<8} {corr_str:<10} {time_str:<8}"
            print(row)
        
        print(f"{'='*120}")

# Molecular configurations database
...
def main():
    ...
    framework = LASVQENOCIFramework(
        gradient_threshold=0.0001,
        vqe_max_cycles=10,
        verbose=1
    )
    
    # Filter configurations for testing (you can modify this list)
    test_molecules = ['H4_STO3G', 'H6_STO3G', 'C4_STO3G', 'C6_STO3G']  # Add or remove as needed
