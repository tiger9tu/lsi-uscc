#!/usr/bin/env python3
"""
Stilbene-120 Energy Comparison - LAS-VQE-NOSI Methods
Based on c6_energy_comparison.py template
Molecular system: Stilbene conformer 120 degrees
"""
...
class Stilbene120EnergyComparison:
    """Energy comparison calculations for Stilbene-120 molecule"""
    
    def __init__(self, basis: str = '6-31g', verbose: int = 1):
        self.basis = basis
        self.verbose = verbose
        
        # Stilbene-120 molecular configuration from mole_lasinfo.txt
        self.geom_file = '../../geom/stil-120.xyz'
        self.ncas_sub = (4, 2, 4)
        self.nelec_sub = ((2, 2), (1, 1), (2, 2))
        self.spin_sub = (1, 1, 1)
        self.frag_atom_list = [[1,2,3,4,5,6,15,16,17,18,19], [0,7,14,20], [8,9,10,11,12,13,21,22,23,24,25]]
        
        # Fragment spin orbital indices for Stilbene-120
        self.frag_spin_orbs = {
    ...
    def setup_molecule(self):
        ...
        print(f"Stilbene-120 molecule setup:")
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                for frag_i, frag_j in frag_pairs:
                    if frag_i in self.frag_spin_orbs and frag_j in self.frag_spin_orbs:
                        ...
                        # For 120° stilbene, significant steric strain but some conjugation
    ...
    def run_las_vqe_protocol2(self, gradient_threshold: float = 0.001, 
        ...
        try:
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                # For 120° stilbene, balance between conjugation and steric effects
                ...
                # Phenyl-vinyl excitations (reduced due to steric strain at 120°)
                for p_orb in phenyl1_orbs[:2]:
                    for v_orb in vinyl_orbs:
                        state_excitations.append((p_orb, v_orb))
                
                for v_orb in vinyl_orbs:
                    for p_orb in phenyl2_orbs[:2]:
                        state_excitations.append((v_orb, p_orb))
                
                # Intra-phenyl excitations (important due to reduced inter-ring coupling)
                for i in range(min(3, len(phenyl1_orbs))):
                    for j in range(i+2, min(5, len(phenyl1_orbs))):
                        state_excitations.append((phenyl1_orbs[i], phenyl1_orbs[j]))
                
                for i in range(min(3, len(phenyl2_orbs))):
                    for j in range(i+2, min(5, len(phenyl2_orbs))):
    ...
    def run_full_comparison(self, threshold: float = 0.001, max_cycle: int = 100) -> List[ComparisonResult]:
        ...
        print("="*70)
        print(f"STILBENE-120 ENERGY COMPARISON - LAS-VQE-NOSI Methods")
    ...
    def write_results_markdown(self, results: List[ComparisonResult], 
        ...
        if filename is None:
            filename = f"STILBENE120_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_{self.basis}.md"
        ...
        markdown_content = f"""# Stilbene-120 Energy Comparison Results

## Calculation Parameters
- **Molecule**: Stilbene conformer 120 degrees
...
## Molecular System
- **Geometry**: `/home/jinx/repo/qchem/las_uccsd_data/stilbene/geometries/stil-120.xyz`
...
### Stilbene-120 Specific Features
- **Dihedral Angle**: 120° between phenyl rings
- **Conjugation**: Significantly reduced π-orbital overlap
- **Electronic Structure**: Approaching cis-stilbene character
- **Steric Effects**: Substantial phenyl-phenyl steric strain
- **Energy Characteristics**: High-energy twisted conformer

### Electronic Structure Implications
- **Fragment Coupling**: Reduced inter-fragment correlation
- **Steric Strain**: Elevated total energy compared to trans form
- **Correlation Patterns**: Mix of conjugation and localized effects
- **Method Challenge**: Balance between delocalization and localization
...
- Handles twisted geometry with moderate fragment coupling

### LAS-VQE-NOSI Protocol 1
- Fragment-based excitation selection
- Reduced inter-fragment excitations due to steric strain
- Emphasis on phenyl ring π-π* transitions

### LAS-VQE-NOSI Protocol 2  
- Systematic excitation selection
- Balanced treatment of steric and electronic effects
- Enhanced intra-fragment correlation for twisted geometry

---
*Generated by Stilbene120EnergyComparison class*
*Calculation completed: {time.strftime('%Y-%m-%d %H:%M:%S')}*
"""
        
        # Write to file
        filepath = f"/home/jinx/repo/qchem/las-uscc-noci-bot/working/sept/stilbene-120/{filename}"
...
def main():
    ...
    print("Stilbene-120 Energy Comparison Study")
    print("=" * 50)
    ...
    for threshold, max_cycle in test_parameters:
        ...
        try:
            ...
            calc = Stilbene120EnergyComparison(basis='6-31g', verbose=1)
            
            # Run comparison
            results = calc.run_full_comparison(threshold=threshold, max_cycle=max_cycle)
            
            # Write results
            filename = f"STILBENE120_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_631g.md"
    ...
    print("\n🎯 Stilbene-120 energy comparison study completed!")
    print("📁 Results written to stilbene-120/ directory")
