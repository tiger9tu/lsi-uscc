#!/usr/bin/env python3
"""
Stilbene-180 Energy Comparison - LAS-VQE-NOSI Methods
Based on c6_energy_comparison.py template
Molecular system: Stilbene conformer 180 degrees (cis-stilbene)
"""
...
class Stilbene180EnergyComparison:
    """Energy comparison calculations for Stilbene-180 molecule"""
    
    def __init__(self, basis: str = '6-31g', verbose: int = 1):
        self.basis = basis
        self.verbose = verbose
        
        # Stilbene-180 molecular configuration from mole_lasinfo.txt
        self.geom_file = '../../geom/stil-180.xyz'
        self.ncas_sub = (4, 2, 4)
        self.nelec_sub = ((2, 2), (1, 1), (2, 2))
        self.spin_sub = (1, 1, 1)
        self.frag_atom_list = [[1,2,3,4,5,6,15,16,17,18,19], [0,7,14,20], [8,9,10,11,12,13,21,22,23,24,25]]
        
        # Fragment spin orbital indices for Stilbene-180
        self.frag_spin_orbs = {
    ...
    def setup_molecule(self):
        ...
        print(f"Stilbene-180 molecule setup:")
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                for frag_i, frag_j in frag_pairs:
                    if frag_i in self.frag_spin_orbs and frag_j in self.frag_spin_orbs:
                        ...
                        # For 180° stilbene (cis), good conjugation but steric strain
                ...
                # Important for cis-stilbene due to compressed geometry
                if state_idx in [0, 2]:  # Phenyl fragments
                    ...
                    if len(frag_orbs) >= 6:
                        ...
                        state_excitations.append((frag_orbs[2], frag_orbs[6]))
                
                # Limit total excitations per state
                if len(state_excitations) > 14:
                    state_excitations = state_excitations[:14]
    ...
    def run_las_vqe_protocol2(self, gradient_threshold: float = 0.001, 
        ...
        try:
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                # For cis-stilbene, emphasize conjugation despite steric strain
                ...
                # Phenyl-vinyl excitations (important for cis conjugation)
                ...
                # Direct phenyl-phenyl excitations (unique to cis geometry)
                for p1_orb in phenyl1_orbs[:2]:
                    for p2_orb in phenyl2_orbs[:2]:
                        state_excitations.append((p1_orb, p2_orb))
                
                # Intra-phenyl excitations (important due to compressed geometry)
                for i in range(min(4, len(phenyl1_orbs))):
                    for j in range(i+2, min(6, len(phenyl1_orbs))):
                        state_excitations.append((phenyl1_orbs[i], phenyl1_orbs[j]))
                
                for i in range(min(4, len(phenyl2_orbs))):
                    for j in range(i+2, min(6, len(phenyl2_orbs))):
                        state_excitations.append((phenyl2_orbs[i], phenyl2_orbs[j]))
                
                # Limit total excitations per state
                if len(state_excitations) > 20:
                    state_excitations = state_excitations[:20]
    ...
    def run_full_comparison(self, threshold: float = 0.001, max_cycle: int = 100) -> List[ComparisonResult]:
        ...
        print("="*70)
        print(f"STILBENE-180 ENERGY COMPARISON - LAS-VQE-NOSI Methods")
    ...
    def write_results_markdown(self, results: List[ComparisonResult], 
        ...
        if filename is None:
            filename = f"STILBENE180_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_{self.basis}.md"
        ...
        markdown_content = f"""# Stilbene-180 Energy Comparison Results

## Calculation Parameters
- **Molecule**: Stilbene conformer 180 degrees (cis-stilbene)
...
## Molecular System
- **Geometry**: `/home/jinx/repo/qchem/las_uccsd_data/stilbene/geometries/stil-180.xyz`
...
### Stilbene-180 Specific Features (Cis-Stilbene)
- **Dihedral Angle**: 180° between phenyl rings (cis configuration)
- **Conjugation**: Maintained π-orbital overlap across vinyl bridge
- **Electronic Structure**: Full phenyl-vinyl-phenyl conjugation
- **Steric Effects**: Maximum phenyl-phenyl steric strain
- **Energy Characteristics**: High-energy due to steric compression but conjugated

### Electronic Structure Implications
- **Conjugation vs. Strain**: Balance between π-delocalization and steric repulsion
- **Fragment Coupling**: Strong inter-fragment correlation despite strain
- **Unique Geometry**: Direct phenyl-phenyl interactions in compressed cis form
- **Correlation Challenge**: Complex interplay of delocalization and steric effects

### Photochemical Relevance
- **Photoisomerization**: Key intermediate in trans-cis stilbene photochemistry
- **Excited States**: Important for understanding photochemical pathways
- **Molecular Motors**: Relevant for stilbene-based molecular devices
- **Steric Control**: Demonstrates steric effects on π-conjugation
...
- Challenges with highly strained but conjugated cis geometry

### LAS-VQE-NOSI Protocol 1
- Fragment-based excitation selection
- Enhanced intra-fragment excitations for compressed geometry
- Focus on conjugation despite steric strain

### LAS-VQE-NOSI Protocol 2  
- Comprehensive excitation selection
- Includes direct phenyl-phenyl interactions unique to cis form
- Complete treatment of conjugation-strain balance

---
*Generated by Stilbene180EnergyComparison class*
*Calculation completed: {time.strftime('%Y-%m-%d %H:%M:%S')}*
"""
        
        # Write to file
        filepath = f"/home/jinx/repo/qchem/las-uscc-noci-bot/working/sept/stilbene-180/{filename}"
...
def main():
    ...
    print("Stilbene-180 Energy Comparison Study")
    print("=" * 50)
    ...
    for threshold, max_cycle in test_parameters:
        ...
        try:
            ...
            calc = Stilbene180EnergyComparison(basis='6-31g', verbose=1)
            
            # Run comparison
            results = calc.run_full_comparison(threshold=threshold, max_cycle=max_cycle)
            
            # Write results
            filename = f"STILBENE180_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_631g.md"
    ...
    print("\n🎯 Stilbene-180 energy comparison study completed!")
    print("📁 Results written to stilbene-180/ directory")
