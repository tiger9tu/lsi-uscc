#!/usr/bin/env python3
"""
Stilbene-60 Energy Comparison - LAS-VQE-NOSI Methods
Based on c6_energy_comparison.py template
Molecular system: Stilbene conformer 60 degrees
"""
...
class Stilbene60EnergyComparison:
    """Energy comparison calculations for Stilbene-60 molecule"""
    
    def __init__(self, basis: str = '6-31g', verbose: int = 1):
        self.basis = basis
        self.verbose = verbose
        
        # Stilbene-60 molecular configuration from mole_lasinfo.txt
        self.geom_file = '../../geom/stil-60.xyz'
        self.ncas_sub = (4, 2, 4)
        self.nelec_sub = ((2, 2), (1, 1), (2, 2))
        self.spin_sub = (1, 1, 1)
        self.frag_atom_list = [[1,2,3,4,5,6,15,16,17,18,19], [0,7,14,20], [8,9,10,11,12,13,21,22,23,24,25]]
        
        # Fragment spin orbital indices for Stilbene-60
        self.frag_spin_orbs = {
    ...
    def setup_molecule(self):
        ...
        print(f"Stilbene-60 molecule setup:")
    ...
    def run_full_comparison(self, threshold: float = 0.001, max_cycle: int = 100) -> List[ComparisonResult]:
        ...
        print("="*70)
        print(f"STILBENE-60 ENERGY COMPARISON - LAS-VQE-NOSI Methods")
    ...
    def write_results_markdown(self, results: List[ComparisonResult], 
        ...
        if filename is None:
            filename = f"STILBENE60_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_{self.basis}.md"
        ...
        markdown_content = f"""# Stilbene-60 Energy Comparison Results

## Calculation Parameters
- **Molecule**: Stilbene conformer 60 degrees
...
## Molecular System
- **Geometry**: `/home/jinx/repo/qchem/las_uccsd_data/stilbene/geometries/stil-60.xyz`
...
### Stilbene-60 Specific Features
- **Dihedral Angle**: 60° between phenyl rings
- **Conjugation**: Partial π-orbital overlap (intermediate between cis/trans)
- **Steric Effects**: Moderate phenyl-phenyl interaction
- **Electronic Structure**: Mixed character between planar and twisted forms

---
*Generated by Stilbene60EnergyComparison class*
*Calculation completed: {time.strftime('%Y-%m-%d %H:%M:%S')}*
"""
        
        # Write to file
        filepath = f"/home/jinx/repo/qchem/las-uscc-noci-bot/working/sept/stilbene-60/{filename}"
...
def main():
    ...
    print("Stilbene-60 Energy Comparison Study")
    print("=" * 50)
    ...
    for threshold, max_cycle in test_parameters:
        ...
        try:
            ...
            calc = Stilbene60EnergyComparison(basis='6-31g', verbose=1)
            
            # Run comparison
            results = calc.run_full_comparison(threshold=threshold, max_cycle=max_cycle)
            
            # Write results
            filename = f"STILBENE60_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_631g.md"
    ...
    print("\n🎯 Stilbene-60 energy comparison study completed!")
    print("📁 Results written to stilbene-60/ directory")
