#!/usr/bin/env python3
"""
Stilbene-90 Energy Comparison - LAS-VQE-NOSI Methods
Based on c6_energy_comparison.py template
Molecular system: Stilbene conformer 90 degrees (orthogonal phenyl rings)
"""
...
class Stilbene90EnergyComparison:
    """Energy comparison calculations for Stilbene-90 molecule"""
    
    def __init__(self, basis: str = '6-31g', verbose: int = 1):
        self.basis = basis
        self.verbose = verbose
        
        # Stilbene-90 molecular configuration from mole_lasinfo.txt
        self.geom_file = '../../geom/stil-90.xyz'
        self.ncas_sub = (4, 2, 4)
        self.nelec_sub = ((2, 2), (1, 1), (2, 2))
        self.spin_sub = (1, 1, 1)
        self.frag_atom_list = [[1,2,3,4,5,6,15,16,17,18,19], [0,7,14,20], [8,9,10,11,12,13,21,22,23,24,25]]
        
        # Fragment spin orbital indices for Stilbene-90
        self.frag_spin_orbs = {
    ...
    def setup_molecule(self):
        ...
        print(f"Stilbene-90 molecule setup:")
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                for frag_i, frag_j in frag_pairs:
                    if frag_i in self.frag_spin_orbs and frag_j in self.frag_spin_orbs:
                        ...
                        # For 90° stilbene, reduced conjugation means fewer excitations needed
                        for orb_i in frag_i_orbs[:3]:  # Limit to first 3 orbs per fragment
                            for orb_j in frag_j_orbs[:3]:
                                if abs(orb_i - orb_j) >= 2:  # Avoid same-orbital excitations
                                    state_excitations.append((orb_i, orb_j))
                
                # Add intra-fragment excitations for phenyl rings (more important at 90°)
    ...
    def run_las_vqe_protocol2(self, gradient_threshold: float = 0.001, 
        ...
        try:
            ...
            for state_idx in range(len(self.frag_spin_orbs)):
                ...
                # For 90° stilbene, emphasize localized excitations
                ...
                # Add specific phenyl ring π-π* excitations (more important at 90°)
                ...
                # Emphasize intra-phenyl excitations for orthogonal geometry
                ...
                # Vinyl-phenyl excitations (reduced but still present)
                for v_orb in vinyl_orbs:
                    state_excitations.append((phenyl1_orbs[0], v_orb))
                    state_excitations.append((v_orb, phenyl2_orbs[0]))
                
                # Limit total excitations per state
                if len(state_excitations) > 16:
                    state_excitations = state_excitations[:16]
    ...
    def run_full_comparison(self, threshold: float = 0.001, max_cycle: int = 100) -> List[ComparisonResult]:
        ...
        print("="*70)
        print(f"STILBENE-90 ENERGY COMPARISON - LAS-VQE-NOSI Methods")
    ...
    def write_results_markdown(self, results: List[ComparisonResult], 
        ...
        if filename is None:
            filename = f"STILBENE90_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_{self.basis}.md"
        ...
        markdown_content = f"""# Stilbene-90 Energy Comparison Results

## Calculation Parameters
- **Molecule**: Stilbene conformer 90 degrees (orthogonal phenyl rings)
...
## Molecular System
- **Geometry**: `/home/jinx/repo/qchem/las_uccsd_data/stilbene/geometries/stil-90.xyz`
...
### Stilbene-90 Specific Features
- **Dihedral Angle**: 90° between phenyl rings (orthogonal geometry)
- **Conjugation**: Minimal π-orbital overlap (broken conjugation)
- **Electronic Structure**: Nearly independent phenyl ring π-systems
- **Steric Effects**: Minimal phenyl-phenyl interaction
- **Energy Characteristics**: Higher energy due to loss of conjugation stabilization

### Electronic Structure Implications
- **Fragment Localization**: Enhanced due to orthogonal geometry
- **Correlation Effects**: Dominated by intra-fragment excitations
- **Inter-fragment Coupling**: Minimal direct π-conjugation
- **Method Performance**: LAS-based methods expected to perform well
...
- Well-suited for orthogonal geometry with natural fragment localization

### LAS-VQE-NOSI Protocol 1
- Fragment-based excitation selection
- Emphasizes intra-fragment excitations for orthogonal geometry
- Reduced inter-fragment excitations due to broken conjugation

### LAS-VQE-NOSI Protocol 2  
- Systematic excitation selection
- Balanced intra- and inter-fragment correlation treatment
- Enhanced phenyl ring π-π* excitations

---
*Generated by Stilbene90EnergyComparison class*
*Calculation completed: {time.strftime('%Y-%m-%d %H:%M:%S')}*
"""
        
        # Write to file
        filepath = f"/home/jinx/repo/qchem/las-uscc-noci-bot/working/sept/stilbene-90/{filename}"
...
def main():
    ...
    print("Stilbene-90 Energy Comparison Study")
    print("=" * 50)
    ...
    for threshold, max_cycle in test_parameters:
        ...
        try:
            ...
            calc = Stilbene90EnergyComparison(basis='6-31g', verbose=1)
            
            # Run comparison
            results = calc.run_full_comparison(threshold=threshold, max_cycle=max_cycle)
            
            # Write results
            filename = f"STILBENE90_ENERGY_COMPARISON_t{threshold}_c{max_cycle}_631g.md"
    ...
    print("\n🎯 Stilbene-90 energy comparison study completed!")
    print("📁 Results written to stilbene-90/ directory")
