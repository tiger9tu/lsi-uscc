...
def amp_spa2spin(t2):
    sign = -1 # for some reason I don't know why
    nocc = t2.shape[0]
    norb = nocc + t2.shape[2]
    a_idxs = []
    i_idxs = []
    amps = []
        ...
        for j in range(t2.shape[1]):
            for a in range(t2.shape[2]):
                for b in range(t2.shape[3]):
                    ...
                    i_idx_buffer = []
                    amp_buffer = []
                    # t2[i,j,a,b] => i->a+nocc  j->b+nocc
                    # alpha beta excitations
                    alpha_from1, alpha_to1, beta_from1, beta_to1 = i, a + nocc, j + norb, b + norb + nocc
                    alpha_from2, alpha_to2, beta_from2, beta_to2 = j, b + nocc, i + norb, a + norb + nocc 

                    a_idx_buffer.extend([[alpha_from1, beta_from1],[alpha_from2,beta_from2]])
                    i_idx_buffer.extend([[alpha_to1, beta_to1],[alpha_to2, beta_to2]])
                    amp_buffer.extend([t2[i,j,a,b], t2[i,j,a,b]])

                    # alpha-alpha / beta-beta excitations
                    if i < j and a < b:
                        anti_symm_amp = t2[i,j,a,b] - t2[i,j,b,a]
                        alpha1_from, alpha1_to, alpha2_from, alpha2_to = i, a + nocc, j, b + nocc
                        beta1_from, beta1_to, beta2_from, beta2_to = i + norb, a + norb + nocc, j + norb, b + norb + nocc
                        
                        a_idx_buffer.extend([[alpha1_from, alpha2_from],[beta1_from, beta2_from]])
                        i_idx_buffer.extend([[alpha1_to, alpha2_to],[beta1_to, beta2_to]])
                        amp_buffer.extend([anti_symm_amp, anti_symm_amp])
                    
                    for a_idx, i_idx, amp in zip(a_idx_buffer, i_idx_buffer, amp_buffer):
                        if any((a_idx == existing_a and i_idx == existing_i) for existing_a, existing_i in zip(a_idxs, i_idxs)):
                            continue
                        a_idxs.append(a_idx)
                        i_idxs.append(i_idx)
                        amps.append(sign * amp)
...
def nci_test(a_idxs_selected, i_idxs_selected, amps, test_config, mol_config, mol,las, mc_uscc, mf):
...
def test(mol_config, test_config,las, mol, mf):
    mp2 = MP2(mf)
    mp2.kernel()
    a_idxs_mp2, i_idxs_mp2, amps_mp2 = amp_spa2spin(mp2.t2)
    a_idxs_mp2 = [np.array(x, dtype=np.uint8) for x in a_idxs_mp2]
    i_idxs_mp2 = [np.array(x, dtype=np.uint8) for x in i_idxs_mp2]
    result = {}
    result['tot_excitation_count'] = len(a_idxs_mp2)

    if VERBOSE >= 2:
        print("mp2 excitations:")
        print_excitations(a_idxs_mp2, i_idxs_mp2)

    mc_uscc = mcscf.CASCI(mf, sum(mol_config['ncas']), sum(mol_config['nelecas']))
    mc_uscc.mo_coeff = las.mo_coeff
    mc_uscc.fcisolver = lasuccsd.FCISolver_USCC(mol, a_idxs_mp2, i_idxs_mp2)
    ...
    if test_config['noci_test']:
        result['las_uscc_noci_eng'], result['las_uscc_noci_vec'], result['min_ci_eng'] = nci_test(a_idxs_mp2, i_idxs_mp2, amps_mp2, test_config, mol_config, mol, las, mc_uscc, mf)
