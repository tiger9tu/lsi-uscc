...
def nci_test(a_idxs_selected, i_idxs_selected, test_config, mol_config, mol,las, mc_uscc, mf):
    ...
    for i in range(nc):
        if( i == nc -1):
            ...
            amplitude = np.random.uniform(-AMPLITUDE, AMPLITUDE)  # random amplitude in [-AMPLITUDE, AMPLITUDE]
...
def test(mol_config, test_config,las, mol, mf):
    ...
    _, _, a_idxs_all, i_idxs_all = grad.get_grad_exact(las, 0.0)
    all_g, g_sel, a_idxs_selected_all, i_idxs_selected_all = grad.get_grad_exact(las, test_config['epsilon'])

    excitation_count = len(a_idxs_selected_all)
    rand_indices = np.random.choice(len(a_idxs_all), excitation_count, replace=False)
    a_idxs_selected = [a_idxs_all[i] for i in rand_indices]
    i_idxs_selected = [i_idxs_all[i] for i in rand_indices]
    ...
    result['tot_excitation_count'] = excitation_count
