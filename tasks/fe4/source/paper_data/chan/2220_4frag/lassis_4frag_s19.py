...
#basis={'C': 'cc-pvdz','H': 'cc-pvdz','S': 'aug-cc-pvdz','Fe': 'aug-cc-pvdz'}
if gpu_run:
    mol=gto.M(atom=mol_name,verbose=4,spin=18,charge=-2,basis=basis, max_memory=500000, output='gpu_lassis_test_s19.log', use_gpu = gpu)
else:
    mol=gto.M(atom=mol_name,verbose=4,spin=18,charge=-2,basis=basis, max_memory=500000, output='cpu_lassis_test_s19.log') #spin = 2S
mf=scf.ROHF(mol)
mf.init_guess='atom'
mf=mf.density_fit()
mf=mf.newton()
...
_,_,conv_mo_coeff,_,_,_ = molden.load('converged.molden')
